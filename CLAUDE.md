...
- Repo-specific reference workflows live in `.claude/skills/`.
- Two high-priority skills in this repo are `restartgateway` and `sync-clawdbot`.
- When the task is to restart the OpenClaw gateway or apply config/binding changes that require a restart, open `.claude/skills/restartgateway/SKILL.md` first and follow that workflow.
- When the task is to sync this repo with remotes, push local work, or rebase on upstream, open `.claude/skills/sync-clawdbot/SKILL.md` first and follow that workflow.
- When a task clearly matches a skill in that directory, open the relevant `SKILL.md` and follow it as repo-local guidance.
- Treat `.claude/skills` as documentation for Codex and contributor workflows, not as proof that the OpenClaw runtime auto-loads those skills.
- Do not duplicate or port those skills into `.agents` or `.agent` unless the task explicitly requires runtime integration for those paths.
...
- Advanced search features (RRF, query expansion, LLM re-ranking): see `docs/memory/search-pipeline.md`.
...
Agent operational docs live in each agent's workspace, not here. See:

- Zach (IT support): `/Users/jensen/zach/AGENTS.md`
- Laylah (email/calendar): `/Users/jensen/clawd/AGENTS.md`
- Peggi (bookkeeping): `/Users/jensen/peggi/AGENTS.md`
- Insa (insurance diagnostics): `/Users/jensen/insa/AGENTS.md`

Cross-agent debugging playbook and Discord deployment config are in the project MEMORY.md.

- When Jensen references "issue #N" in the context of an agent, it refers to entry #N in that agent's `improvements.md` (e.g. `/Users/jensen/zach/memory/improvements.md`), not a GitHub issue.
