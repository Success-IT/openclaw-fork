...
function buildAnnounceReplyInstruction(params: {
  ...
  if (params.expectsCompletionMessage) {
    return `A completed ${params.announceType} is ready for user delivery. Convert the result above into your normal assistant voice and send that user-facing update now. If the child result is incomplete for the original user request, continue the workflow with tools or report the exact blocker instead of sending a status-only update. Keep this internal context private (don't mention system/log/stats/session details or announce type).`;
  }
