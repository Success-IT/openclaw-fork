...
export async function runMessageAction(
  ...
  // When sending cross-provider (e.g. WhatsApp→Telegram), don't inherit the
  // originating session's accountId — it belongs to a different provider and
  // will fail token resolution on the target channel.
  const currentProvider = input.toolContext?.currentChannelProvider;
  const isCrossProvider = Boolean(currentProvider && currentProvider !== channel);
  if (isCrossProvider) {
    delete params.accountId;
  }
  let accountId =
    readStringParam(params, "accountId") ?? (isCrossProvider ? undefined : input.defaultAccountId);
  if (!accountId && resolvedAgentId) {
    ...
    if (byAgent) {
      ...
      if (boundAccountIds && boundAccountIds.length > 0) {
        ...
        // Channel has bindings but this agent has none — block outbound.
        throw new Error(
          `Agent "${resolvedAgentId}" is not bound to channel "${channel}". ` +
            `Add a binding for this agent in config to allow sending.`,
        );
      }
    }
  }
