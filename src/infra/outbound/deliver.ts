...
import { isLikelyGarbledOutput, sanitizeGarbledOutput } from "./garbled-output-guard.js";
import type { OutboundIdentity } from "./identity.js";
import type { DeliveryMirror } from "./mirror.js";
import type { NormalizedOutboundPayload } from "./payloads.js";
import { normalizeReplyPayloadsForDelivery } from "./payloads.js";
import { isPlainTextSurface, sanitizeForPlainText, stripThinkingBlocks } from "./sanitize-text.js";
...
function normalizePayloadsForChannelDelivery(
  ...
  for (const payload of normalizeReplyPayloadsForDelivery(payloads)) {
    ...
    // Strip thinking/reasoning blocks unconditionally on ALL surfaces.
    // Models may emit <thinking>, <think>, or <reasoning> tags that should
    // never reach end users regardless of channel type.
    if (sanitizedPayload.text) {
      const stripped = stripThinkingBlocks(sanitizedPayload.text);
      if (stripped !== sanitizedPayload.text) {
    ...
    // Suppress garbled model output (corrupted directives, random bracket/escape
    // noise) before it reaches any channel. Applies to all surfaces.
    if (sanitizedPayload.text && isLikelyGarbledOutput(sanitizedPayload.text)) {
      const replacement = sanitizeGarbledOutput(sanitizedPayload.text);
      if (replacement == null) {
        continue;
      }
      sanitizedPayload = { ...sanitizedPayload, text: replacement };
    }
