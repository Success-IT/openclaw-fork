...
describe("stripThinkingBlocks", () => {
  it("strips <thinking> blocks", () => {
    expect(stripThinkingBlocks("before<thinking>internal reasoning</thinking>after")).toBe(
      "beforeafter",
    );
  });

  it("strips <think> blocks", () => {
    expect(stripThinkingBlocks("before<think>internal reasoning</think>after")).toBe("beforeafter");
  });

  it("strips <reasoning> blocks", () => {
    expect(stripThinkingBlocks("before<reasoning>internal reasoning</reasoning>after")).toBe(
      "beforeafter",
    );
  });
  ...
  it("is case-insensitive", () => {
    expect(stripThinkingBlocks("<THINKING>test</THINKING>ok")).toBe("ok");
    expect(stripThinkingBlocks("<Think>test</Think>ok")).toBe("ok");
  });

  it("strips multiple blocks", () => {
    expect(stripThinkingBlocks("a<think>x</think>b<reasoning>y</reasoning>c")).toBe("abc");
  });

  it("passes through text without thinking blocks", () => {
    expect(stripThinkingBlocks("normal text")).toBe("normal text");
  });
});
