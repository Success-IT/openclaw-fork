 ...
 *
 * Also strips reasoning/thinking blocks that should never be visible to users.
 *
 ...
 */
/**
 * Strip `<thinking>`, `<think>`, `<reasoning>` blocks from any text.
 * Called unconditionally for ALL delivery surfaces (not just plain-text).
 */
...
export function sanitizeForPlainText(text: string): string {
  return (
    stripThinkingBlocks(text)
      // Strip common reasoning patterns that leak without tags
      .replace(
        /^(Looking at|I need to analyze|Let me think|Analyzing|Processing|Evaluating|Considering):.*$/gim,
        "",
      )
      // Strip HEARTBEAT_OK and variants - these are internal acknowledgments, not user-facing messages
      .replace(/^HEARTBEAT_OK.*$/gim, "")
      .replace(/^\[System Message\].*HEARTBEAT_OK.*$/gim, "")
