...
describe("web processMessage inbound context", () => {
  ...
  it("includes group tier and system prompt in inbound context", async () => {
          ...
          channels: {
            whatsapp: {
              groups: {
                "123@g.us": {
                  tier: "trusted",
                  systemPrompt: "Handle this as a managed support group.",
                },
              },
            },
          },
    ...
    const ctx = capturedCtx as any;
    expect(ctx.GroupSystemPrompt).toBe("Handle this as a managed support group.");
    expect(ctx.GroupTier).toBe("trusted");
  });
